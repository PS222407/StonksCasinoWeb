                            ...
                            <div class="row">
                                ...
                                @else
                                    <form action="{{ route('selectcard')}}" method="post">
                                        @csrf
                                        <input type="text" name="cardskin" id="cardskin" hidden
                                               value="{{$card_skin->id}}">
                        ...
                        @else
                            <div class="row">
                                ...
                                @if ($test == true)
                                    ...
                                    @endif
                                @else
                                    <label><img src="{{asset('img/Token.png')}}" class="imgtoken"
                                                alt=""> {{$card_skin->price}}</label>
                            </div>
                            <div class="row">
                                <button class="btn btn-groen" data-toggle="modal" data-target="#Winkelwagen{{$amount}}">
                                    Kopen
                                </button>
                                <div class="modal fade" id="Winkelwagen{{$amount}}" tabindex="-1" role="dialog"
                                     aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                                    <div class="modal-dialog modal-dialog-centered" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header card-groen">
                                                <h5 class="modal-title" id="exampleModalLongTitle">Weet u zeker dat u
                                                    dit wil kopen</h5>
                                            ...
                                            <div class="modal-body card-registreer">
                                                <div class="row  mx-auto d-flex">
                                                    <div class="col col-sm-6 ">
                                                        <img class="imgshop" src="{{ asset($card_skin->web_img_path) }}"
                                                             alt="">
                                                    </div>
                                                    <div class="col col-sm-6 pl-1 d-flex align-items-center ">
                                                        <div>
                                                            <h2>{{$card_skin->name}}</h2>
                                                            <h4>prijs: <img src="{{asset('img/Token.png')}}"
                                                                            class="imgtoken"
                                                                            alt=""> {{$card_skin->price}}</h4>
                                                            <div class="d-flex">
                                                                ...
                                                                <form action="{{route('buycard')}}" method="post">
                                                                    @csrf
                                                                    <input type="text" hidden name="buycard1"
                                                                           id="buycard1" value="{{$card_skin->id}}">
                                                                    <button type="submit" class="btn btn-groen">Kopen
                                                                    </button>
                                                                </form>
                                                            </div>
                                                        </div>
